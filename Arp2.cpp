...
int main() {
    ...
    int n;
    std::cout << "Digite o n�mero de alimentos a serem inseridas na sua despensa: ";
    std::cin >> n;
    ...
    if (index_sequential == -1) {
        std::cout << "Alimento n�o encontrado (busca sequencial)" << std::endl;
    }
    else {
        std::cout << "Alimento encontrado na posi��o " << index_sequential << " (busca sequencial)" << std::endl;
    }

    int index_binary = binary_search(frutas, nome_busca);
    if (index_binary == -1) {
        std::cout << "Alimento n�o encontrado (busca bin�ria)" << std::endl;
    }
    else {
        std::cout << "Alimento encontrado na posi��o " << index_binary << " (busca bin�ria)" << std::endl;
    }
